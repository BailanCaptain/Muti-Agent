...
```markdown
## 交给范德彪 Review: acceptance-guardian 运行时注入

### What
在 context-assembler 中实现 acceptance-guardian 零上下文模式：
- 新增 POLICY_GUARDIAN（零上下文策略）
- assemblePrompt 检测 guardianMode → 替换 system prompt
- flushDispatchQueue 中 skill match 检测 acceptance-guardian → 启用 guardian 模式

### Why
acceptance-guardian 的核心设计是"零上下文消除确认偏差"——agent 不能带实现记忆去验收。
...
### Open Questions
1. 当前检测方式是 skillHint 包含 "acceptance-guardian"，是否需要更精确的匹配？
2. guardian 模式下 MCP 工具是否也应该受限？

### Next Action
请 review 这三个文件：
1. context-policy.ts — POLICY_GUARDIAN 定义
2. context-assembler.ts — guardianMode 分支
