...
### Fail-closed 结论词禁令（对齐家规 §P5）

**未拿出本轮证据前，禁止**在消息/报告中使用：
`fixed` · `完成` · `没问题` · `确认` · `一定是` · `搞定` · `pass 了`

**允许**：`我推测` · `我计划` · `目前看起来` · `已改 + 待验证`

**"本轮证据"的硬标准**（三选一即可，缺一即 fail-closed）：

1. 新文件 + 行号（`path/to/file.ts:L42-58`）——证明代码真改了
2. 新测试命令输出（`pnpm test → 12 passed, 0 failed`）——证明行为真通过
3. 新实测输出（curl / UI 截图 / 日志片段）——证明端到端真跑通

**UX / 前端专项**：涉及前端 / UI / 页面交互的改动，"新实测输出"必须是**浏览器级证据**，不接受"代码通读过"或"本地看了一眼"。对齐家规 §P5 末段与 `debugging` Bug Report 第 6 条。两条路径（F038 起优先 ①）：

1. **E2E 自动化（首选）**：走 `webapp-testing` skill — `pnpm test:e2e` 回归全绿输出；
   涉及**前后端交互**的 feature 还须有该交互的专项 E2E 用例（真点击/fill + 后端独立断言）+ 一次验红记录
2. **人工浏览器实操 + 截图**（E2E 覆盖不到的场景：视觉效果 / 复杂拖拽 / 多窗口）：用户路径走一遍留截图

> **纯视觉 / 样式 / restyle 改动**：证据形态 = `design-taste` 的量化审计前后对比（裸色/任意字号/
> emoji 图标计数归零）+ 多状态截图（空态/聚焦/各面板）。改前没立基线 = 无法证明"变好看"，按未完成处理。

> 只换说法 / 复制上次输出 / "应该" / "probably" → 不算证据，直接按**未完成**处理。
