...
**前后端交互 AC 专项（F038）**：AC 涉及「页面操作 → API → 落库 → UI 更新」链路的，
只有单测/代码通读**不算验收**——必须有浏览器级证据之一：

- 复跑该 feature 的专项 E2E 用例（`webapp-testing` skill，`pnpm test:e2e` 本次输出），或
- 守护 agent 自己浏览器实操走一遍用户路径 + 截图

作者只交单测证据 → 该 AC 判 ❌，附「缺浏览器级证据」。
