...
计数单位：**同一 bug / feature**（和家规 §17 字面一致；不按 finding 拆计数，避免 drift）：
- author 在同一 bug/feature 声称 fixed/完成但复验失败 → +1 次假绿
- author 回复没新文件+行号 / 新测试 / 新实测输出 → +1 轮无证据增量

**阈值（任一）**：假绿 ≥ 2 次 / 无证据增量 ≥ 3 轮 → **必须**在本轮回复行首宣布：

    @<author> TAKEOVER — 家规 §17（假绿 N / 无证据增量 N 轮）
    请交 handoff 四件套：复现步骤 / 已尝试项 / 失败原因 / 当前怀疑点
    由 @<第三方 agent> 接手。
