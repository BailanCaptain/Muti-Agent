...
大 scope feature **每个 Phase merge 后**，主动和小孙碰头（不是问"要不要继续"，是确认方向）：
...
**⚠️ Phase 级进度由 `merge-gate` Phase 文档同步实时更新**：每次 PR merge 后，merge-gate 负责更新 AC 打勾、Timeline 记录。Completion 阶段不需要补这些——它们应该已经是最新的。

**交付物核实铁律**：spec checkbox 是记录工具，不是真相源。声称"完成"或"未完成"前，**必须**核实实际 commit/PR 状态（`git log --grep` + `gh pr list`）。
...
**独立验收证物对照表**（守护 agent 必须输出此格式，否则 BLOCKED）：

```markdown
| 小孙原话（逐字引用） | 当前实际状态（代码/测试/命令输出） | 匹配？ |
|---------------------|-------------------------------|--------|
| "我要 XXX"           | [代码位置 + 测试名]              | ✅     |
| "不想看到 YYY"       | [验证 YYY 不存在的证据]           | ✅     |
```

**BLOCKED 条件**（任一触发 → 不放行）：
- 守护 agent 输出缺少对照表 → BLOCKED
- 对照表中有未匹配项（❌）→ BLOCKED，踢回修改
- 找不到小孙原话（Discussion 缺失）→ BLOCKED，要求补充
...
- 开发完成后 → `quality-gate` → `acceptance-guardian` → `requesting-review`
