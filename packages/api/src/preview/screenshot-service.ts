...
function resolvePlaywrightNodeModules(): string {
  ...
  try {
    const out = execFileSync(
      "node",
      ["-e", "process.stdout.write(require.resolve('playwright'))"],
      {
        encoding: "utf-8",
        timeout: 5000,
  ...
  try {
    const out = execFileSync(
      "npx",
      ["--yes", "node", "-e", "process.stdout.write(require.resolve('playwright'))"],
      {
        encoding: "utf-8",
        timeout: 15000,
        shell: true,
...
export async function captureScreenshot(
  ...
  const { stdout } = await execFileAsync(process.execPath, ["-e", PLAYWRIGHT_SCRIPT], {
    timeout: 30000,
