...
test("classifyFailure routes Gemini RESOURCE_EXHAUSTED to context_exhausted (B002)", () => {
  ...
  assert.equal(
    result.shouldClearSession,
    true,
    "B002: must clear session so next turn starts fresh with rolling summary",
  )
...
test("classifyFailure detects stall-killed error from runtime marker", () => {
  const message = "[runtime] Agent 进程看起来已卡住（CPU 空转，无新输出 ≥ 180 秒）。请重试一次。"
  const result = classifyFailure("", message)
...
test("classifyFailure falls through to 'unknown' for unrecognized errors", () => {
  ...
  assert.equal(
    result.shouldClearSession,
    false,
    "F004: unknown must preserve session — clearing throws away history",
  )
  assert.equal(result.safeToRetry, true)
})
...
test("classifyFailure handles both haystacks (stderr + errorMessage) together", () => {
  ...
  assert.equal(
    result.class,
    "context_exhausted",
    "quota exhaustion is capacity-like, clear session to be safe",
  )
  assert.equal(result.shouldClearSession, true)
})
...
test("classifyFailure is case-insensitive for English patterns", () => {
  ...
  assert.equal(
    classifyFailure("Rate Limit Exceeded", "").class,
    "rate_limited",
    "plain RPS rate-limit stays in rate_limited",
  )
...
test("B017: Gemini CLI 'Invalid session identifier' → session_corrupt", () => {
  ...
  const stderr =
    'Error resuming session: Invalid session identifier "00000000-0000-0000-0000-000000000000". Searched for sessions in ...'
  const result = classifyFailure(stderr, "")
  assert.equal(
    result.class,
    "session_corrupt",
    "must catch Gemini 'invalid session identifier' wording (Gemini's only defence)",
  )
  assert.equal(result.shouldClearSession, true)
})
