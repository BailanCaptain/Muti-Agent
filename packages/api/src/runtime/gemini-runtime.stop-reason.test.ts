...
describe("GeminiRuntime.parseStopReason", () => {
  ...
  it("maps result.finishReason=STOP → complete", () => {
    assert.equal(runtime.parseStopReason({ type: "result", finishReason: "STOP" }), "complete")
  })

  it("maps result.finishReason=END_TURN → complete", () => {
    assert.equal(runtime.parseStopReason({ type: "result", finishReason: "END_TURN" }), "complete")
  })
  ...
  it("maps result.finishReason=SAFETY → refused", () => {
    assert.equal(runtime.parseStopReason({ type: "result", finishReason: "SAFETY" }), "refused")
  })

  it("maps result.finishReason=RECITATION → refused", () => {
    assert.equal(runtime.parseStopReason({ type: "result", finishReason: "RECITATION" }), "refused")
  })
  ...
  it("returns null for unknown finishReason", () => {
    assert.equal(runtime.parseStopReason({ type: "result", finishReason: "WHATEVER" }), null)
  })
  ...
  it("returns null when result has no finishReason at all", () => {
    assert.equal(runtime.parseStopReason({ type: "result", status: "success" }), null)
  })
})
