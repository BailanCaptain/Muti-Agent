...
test("B017: findSessionId skips nested error-result (recursive case)", () => {
  ...
  assert.equal(
    findSessionId(event),
    null,
    "nested error-result junk session_id must not be captured",
  )
})
...
test("B017: findSessionId still returns session_id from a normal successful result", () => {
  ...
  assert.equal(
    findSessionId(event),
    "real-success-id",
    "successful result session_id must still be captured",
  )
})
...
test("B017: findSessionId still returns session_id from system init event", () => {
  ...
  assert.equal(
    findSessionId(event),
    "real-init-id",
    "system-init session_id must still be captured",
  )
})
