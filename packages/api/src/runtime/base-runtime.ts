...
export function buildCallbackPrompt(agentLabel: string) {
  return [
    ...
    "node -e \"const base=process.env.MULTI_AGENT_API_URL; const id=process.env.MULTI_AGENT_INVOCATION_ID; const token=process.env.MULTI_AGENT_CALLBACK_TOKEN; fetch(`${base}/api/callbacks/thread-context?invocationId=${encodeURIComponent(id)}&callbackToken=${encodeURIComponent(token)}`).then(r=>r.text()).then(console.log)\"",
    "",
    "Post a public room message example:",
    "node -e \"const base=process.env.MULTI_AGENT_API_URL; const id=process.env.MULTI_AGENT_INVOCATION_ID; const token=process.env.MULTI_AGENT_CALLBACK_TOKEN; const content=process.argv.slice(1).join(' '); fetch(`${base}/api/callbacks/post-message`, {method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ invocationId:id, callbackToken:token, content })}).then(r=>r.text()).then(console.log)\" \"Your message here\"",
    "",
