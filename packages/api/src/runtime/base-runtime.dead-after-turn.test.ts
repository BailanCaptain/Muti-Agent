...
test("B009-B: deadProcess + turnCompleted → resolves with the agent's response", async () => {
  ...
  assert.equal(
    result.stopReason,
    "complete",
    "stopReason must be 'complete' when turn finished before death",
  )
  assert.equal(result.exitCode, null, "exitCode should be null (killed process)")
  assert.ok(
    result.rawStdout.includes("turn.completed"),
    "rawStdout must contain the turn.completed line",
  )
})
...
test("B009-B: deadProcess without turnCompleted → still rejects", async () => {
  ...
  await assert.rejects(
    handle.promise,
    /异常退出|dead/i,
    "must reject when process dies without turn.completed",
  )
})
...
test("B009-B: turnCompleted flag ignores non-JSON stdout lines", async () => {
  ...
  await assert.rejects(
    handle.promise,
    /异常退出|dead/i,
    "non-JSON turn.completed text must not trigger resolve",
  )
})
...
test("B009-B: timeout/stall without turnCompleted still rejects (no regression)", async () => {
  ...
  await assert.rejects(
    handle.promise,
    /睡着了|重试一次/,
    "timeout without turnCompleted must still reject",
  )
