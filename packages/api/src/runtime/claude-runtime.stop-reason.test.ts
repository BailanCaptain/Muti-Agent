...
describe("ClaudeRuntime.parseStopReason", () => {
  ...
  it("maps result.stop_reason=end_turn → complete", () => {
    assert.equal(runtime.parseStopReason({ type: "result", stop_reason: "end_turn" }), "complete")
  })
  ...
  it("maps result.stop_reason=refusal → refused", () => {
    assert.equal(runtime.parseStopReason({ type: "result", stop_reason: "refusal" }), "refused")
  })

  it("maps result.stop_reason=tool_use → tool_wait", () => {
    assert.equal(runtime.parseStopReason({ type: "result", stop_reason: "tool_use" }), "tool_wait")
  })
  ...
  it("returns null for unknown stop_reason values", () => {
    assert.equal(runtime.parseStopReason({ type: "result", stop_reason: "something_new" }), null)
  })
  ...
  it("returns null when message_delta has no nested stop_reason", () => {
    assert.equal(runtime.parseStopReason({ type: "message_delta", delta: {} }), null)
  })
})
