...
function detectCodexProfile(homeDir: string): ProviderProfile {
  ...
  const cache = readJson<{ models?: Array<{ slug?: string; display_name?: string }> }>(
    modelsCachePath,
  )
  const cacheModels =
...
export function listProviderProfiles(): ProviderProfile[] {
  ...
  return [detectCodexProfile(homeDir), detectClaudeProfile(homeDir), detectGeminiProfile(homeDir)]
}
