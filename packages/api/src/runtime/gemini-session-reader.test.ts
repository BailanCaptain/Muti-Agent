...
describe("readGeminiThoughtsFromSession — real CLI layouts", () => {
      ...
      messages: [
        {
          id: "g1",
          type: "gemini",
          thoughts: [{ subject: "NewLayout", description: "reads from UUID subdir" }],
        },
      ],
    })
  ...
  it("returns [] when no file has a matching sessionId", async () => {
    ...
    const thoughts = await readGeminiThoughtsFromSession("nonexistent-sid-uuid-yyyy", {
      home,
      projectDir: "multi-agent",
    })
  ...
  it("jsonl layout: reads thoughts from per-line events matching sessionId (CLI ≥ 04-23)", async () => {
    ...
    writeFileSync(join(chats, `session-2026-04-23T12-34-${sid.slice(0, 8)}.jsonl`), raw)
  ...
  it("jsonl layout: skips malformed / partial event lines without throwing", async () => {
    ...
    const content = [
      ...
      JSON.stringify({
        id: "g1",
        type: "gemini",
        thoughts: [{ subject: "Recovered", description: "after partial line" }],
      }),
    ].join("\n")
    writeFileSync(join(chats, "session-2026-04-23T12-34-partial1.jsonl"), content)
  ...
  it("jsonl layout: picks last gemini event when multiple exist", async () => {
    ...
    writeFileSync(join(chats, "session-2026-04-23T12-40-multi111.jsonl"), content)
