...
describe("GeminiRuntime — skill detection from tool parameters", () => {
  ...
  it("classifies read_file on multi-agent-skills/ as skill", () => {
    const event = runtime.transformToolEvent({
      ...
      tool_name: "read_file",
      parameters: {
        file_path: "C:/Users/-/Desktop/Multi-Agent/multi-agent-skills/cross-role-handoff/SKILL.md",
      },
    })
    assert.ok(event)
  ...
  it("classifies read_file with backslash path as skill", () => {
    const event = runtime.transformToolEvent({
      ...
      parameters: {
        file_path:
          "C:\\Users\\-\\Desktop\\Multi-Agent\\multi-agent-skills\\cross-role-handoff\\SKILL.md",
      },
    })
    assert.ok(event)
    assert.equal(event.source, "skill")
  ...
  it("classifies Get-Content with backslash skill path as skill", () => {
    const event = runtime.transformToolEvent({
      ...
      item: {
        type: "command_execution",
        command:
          'Get-Content -Path "C:\\Users\\-\\Desktop\\Multi-Agent\\multi-agent-skills\\cross-role-handoff\\SKILL.md"',
      },
    })
    assert.ok(event)
    assert.equal(event.source, "skill")
