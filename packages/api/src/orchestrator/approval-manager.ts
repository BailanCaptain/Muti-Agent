...
export class ApprovalManager {
    ...
    return new Promise<ApprovalResult>((resolve) => {
      const timer = setTimeout(() => {
        ...
        this.emit({
          ...
          payload: { sessionGroupId: request.sessionGroupId, requestId, granted: false },
        })
  ...
  respond(requestId: string, granted: boolean, scope: ApprovalScope): boolean {
    ...
    this.emit({
      type: "approval.resolved",
      payload: { sessionGroupId: entry.request.sessionGroupId, requestId, granted },
    })
  ...
  cancelAll(sessionGroupId: string): void {
    for (const [requestId, entry] of this.pending) {
      if (entry.request.sessionGroupId === sessionGroupId) {
        ...
        this.emit({
          type: "approval.resolved",
          payload: { sessionGroupId, requestId, granted: false },
        })
