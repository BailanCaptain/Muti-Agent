...
export const COMMUNITY_NOISE_RE: RegExp = new RegExp(
  [
    ...
    // 生活杂事要求叙事结果成对出现；领域本身可能有真实 AI 研究（如相亲推荐模型评测）
    "(?:周末|最后|一个人|吃了|对象|经历|奇葩|踩坑|踩的坑).{0,40}(?:相亲|约会|结婚|彩礼|房贷|买房|租房|装修|理财|存款|火锅)|(?:相亲|约会|结婚|彩礼|房贷|买房|租房|装修|理财|存款|火锅).{0,40}(?:周末|最后|一个人|吃了|对象|经历|奇葩|踩坑|踩的坑)",
    // 隐蔽个人求助：不出现“求助/迷茫”，但用个人职业处境 + 面向社区选项征询表达
    "(?:刚毕业|应届|offer|岗位).{0,32}(?:大家|你们|各位).{0,16}(?:会选|选哪个|怎么选|如何选|建议)",
    // 隐蔽职场抱怨：AI 只是工作背景，正文核心是加班、工资或与同辈比较
    "(?:每天|天天|经常|一直).{0,24}(?:加班|熬夜).{0,48}(?:工资|薪资|待遇|同学|朋友|不如|没有|低|少)|(?:加班|熬夜).{0,48}(?:工资|薪资|待遇|同学|朋友)|(?:我|自己|同学|朋友).{0,48}(?:工资|薪资|待遇).{0,48}(?:不如|没有|低|少)|(?:工资|薪资|待遇).{0,48}(?:同学|朋友|不如)",
...
/**
 * 最终发布门专用的高置信正文噪声判定。这里刻意不复用上面的宽标题词表扫描全文：
 * 技术长文正文偶尔提到“工资/买房”并不代表主题是生活帖。只有个人处境与征询、
 * 职场抱怨或生活叙事形成成对语义时才命中，用来堵住“中性技术标题 + 噪声正文”。
 */
export const COMMUNITY_HIGH_CONFIDENCE_SOURCE_NOISE_RE: RegExp = new RegExp(
  [
    // 个人职业处境 + 面向社区征询选择
    "(?:刚毕业|应届|在校|offer|岗位).{0,80}(?:大家|你们|各位).{0,24}(?:会选|选哪个|怎么选|如何选|建议)",
    // 持续加班/熬夜，或与工资、同辈比较形成抱怨叙事
    "(?:每天|天天|经常|一直).{0,24}(?:加班|熬夜).{0,48}(?:工资|薪资|待遇|同学|朋友|不如|没有|低|少)|(?:加班|熬夜).{0,48}(?:工资|薪资|待遇|同学|朋友)|(?:工资|薪资|待遇).{0,48}(?:同学|朋友|不如|没有|低|少)",
    // 个人生活叙事；AI 只作为规划工具或背景时仍不属于 AI 研究/工程讨论
    "(?:周末|最后|一个人|吃了|对象|经历).{0,80}(?:相亲|约会|结婚|彩礼|火锅)|(?:相亲|约会|结婚|彩礼|火锅).{0,80}(?:周末|最后|一个人|吃了|对象|经历)",
  ].join("|"),
  "i",
)

export function isHighConfidenceCommunitySourceNoiseItem(item: NormalizedItem): boolean {
  return COMMUNITY_HIGH_CONFIDENCE_SOURCE_NOISE_RE.test(
