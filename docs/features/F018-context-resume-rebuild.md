...
- [~] AC3.5 生产触发: `message-service` 在真实 turn 里填充 bootstrap metadata 的调用点延 P4（与 seal hook 同批；按 B 方案决策：P3 不扩 scope 做 wiring，避免先 wire 再改）
- [x] AC3.6: 单测覆盖各区段优先级与 drop 顺序（12 bootstrap 单测 + 3 assembler 集成）
- [~] AC3 集成生产触发：message-service 在 seal 后将 TranscriptWriter digest / ThreadMemory / sessionChainIndex 填入 assemblePrompt 的触发点延到 P4（与 seal hook 同批，避免先 wire 再改）
