...
- **[B019](../bugReport/B019-f018-embedding-huggingface-offline.md)（P1，open，2026-04-25）** — 模块六本地 embedding 永久不可用。`@huggingface/transformers` 默认 pipelineLoader 直拉 `huggingface.co` 撞大陆网络墙，每条消息触发 ensureModel 都 ConnectTimeout 静默降级，`message_embeddings` 表 0 行。代码逻辑/wiring 无误，缺的是模型权重的离线/本地分发路径。修复后 F018 模块六才真正 active；在此之前 `recall_similar_context` 工具永远返回空 hits（不影响主流程，符合 AC6.5 静默降级铁律，但跨 session 语义召回完全失效）。
