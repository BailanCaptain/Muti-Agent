...
| 2026-04-18 | P5 Codex adversarial-review 4 轮迭代 — Round 1: 2 HIGH+1 MEDIUM（hits sanitize 绕过 / ensureModel 并发丢事件 / blanket catch 无日志）；Round 2: 1 MEDIUM（内部 catch 静默）；Round 3: 1 MEDIUM（early-return 无 caller context）；Round 4: **APPROVE** + 1 residual non-blocking 补测。746/746 tests 绿 |
| 2026-04-18 | **P5 merged** — e34bcaf ff merge 至 origin/dev（rebase on F019 close）。单 backend 双 frontend 模型落地：/api/callbacks/recall-similar-context + Claude 原生 MCP + Codex/Gemini prompt 注入；message-service post-message fire-and-forget generateAndStore；完整 logger observability。F007 AC5.2/5.5 "虚标"正式闭环 |
...
- ~~**B019**（P1，2026-04-25）模块六本地 embedding 永久不可用~~ — **closed 2026-04-25**：[B019](../bugReport/B019-f018-embedding-huggingface-offline.md) 走本地权重分发 + pipelineLoader 离线优先修复。**修复证物**（外部观测 dogfood）：e2e 临时库 `SELECT COUNT(*) FROM message_embeddings = 3` ✅ / recall 召回 top score=0.3662 ✅ / `model-not-ready` warn 0 条 ✅ / API listen ready ✅。F018 模块六从「代码 wiring + tests 绿但生产调用 0」二阶虚标转为真生效。post-merge 7 天复查窗：2026-05-02。
