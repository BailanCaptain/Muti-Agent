...
| 2026-07-10 | **DONE：德彪 r8 GO → 合并 dev → 主运行时真机全链闭环（小孙验收）**。德彪 r8 **GO 零新洞**（435k tokens：8803 真 HTTP GET/HEAD/304 三头实测 / 内存真 gateway 四路重放 / 1MiB 最坏转义 6,293,123<信封实跑 / dot-segment 逃逸返 null；四洞逐条判闭合）→ 小孙拍板合入。**合并**：feature worktree `merge origin/dev`（12 commit 含 F039 重构）解 3 前端冲突（`--theirs` F039 新样式为底 + `git diff -w` 提 F040 手机适配净改动回贴 composer/sidebar/timeline）→ tag `f040-pre-squash` 锚 75+merge 历史 → `reset --soft` squash → 树等价 0 差异 + 倒灌 comm 空集双核验 → push **squash `48aaa4e`**。**主运行时收尾 4 修**（真机暴露的老账，全非 F040 本体）：① dist 补建 `a7f64fb`（start-project 只跑 tsx 从不建 dist，MCP 工具面冻在 6-12 半月，send_file 缺失=实锤 → 启动链补 tsc）② 手机生产构建 `f34cdc8`（dev 首屏 6MB 未压缩走 Tailscale 香港中继必超时「服务器停止响应」→ 切 next start，实测 353KB gzip 17× 瘦身；web-prod-build-check 只在前端变更时重建）③ CORS .env 加载 `e31b773`（API 只读 process.env 而 start-project 从不灌 .env → CORS_ORIGIN 恒 undefined 回落 localhost-only → 手机 origin 被吞数据全空；for/f 灌 .env，实测 CRLF 自剥 CR）④ 构建检查 git 化 `66d5678`（②的 Get-ChildItem -Recurse 撞 PS5.1 递归陷阱死循环卡死 start-project → 改 git-diff 判前端变更，0.7s 零文件系统遍历）。**真机全链验收（小孙）**：私聊图文双向 ✅ / 群图 owner 通道 ✅ / 出站文件 send_file ✅ / 手机 PWA 会话+渠道数据 ✅。**遗留 TD（不阻塞）**：主运行时 .env 除飞书外变量历史全空转（本次补上）；群裸文件入站平台不推（走私聊/@）；worktree 待清（tag 留证）。**教训**：真机联调把三个半月的潜伏账一次性炸出——「桌面能用」不等于「配置真生效」（默认值恰好一致会掩盖空转）；给自己埋的雷（构建检查死循环）自己排，红先纪律对 infra 脚本同样适用 | 2026-07-10 |
