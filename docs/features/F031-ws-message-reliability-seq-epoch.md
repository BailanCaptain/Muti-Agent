...
- HTTP 快照响应（bootstrap / selectSessionGroup 拉的 group snapshot）附带当前组水位线；
...
- [ ] AC2: 快照水位线：HTTP group snapshot 响应携带 `{ epoch, seq }`；客户端以水位线换基线，seq ≤ 水位线的流事件丢弃不误报
- [ ] AC3: 客户端 gap 检测：连续通过 / 跳号触发 catch-up（复用 selectSessionGroup 全量重拉）+ console.warn 丢失区间 / 陈旧 seq 丢弃 / epoch 变化重置 + 全量重拉；catch-up debounce + 失败重试上限 + 降级；单测覆盖
- [ ] AC4: 集成测试：模拟 socket evict 丢广播 → gap 检测 → catch-up 收敛；服务端重启（epoch 变化）→ 重置收敛；同组双 socket 场景一方收直发事件**不**造成另一方假 gap
- [ ] AC5: 可观测性：gap 事件客户端有结构化 console.warn（组 / epoch / 丢失区间 / 触发的动作），让"消息没显示"从玄学变成一行日志
...
| 快照水位线 | 必须携带 | 无水位线则流与快照无法对齐，切房间/重连必误报 |
