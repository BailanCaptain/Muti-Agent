...
| F037 | 日报邮件推送系统（DailyBrief）：每日 07:30 中文 HTML 日报（AI/社区动态/今日热点/GitHub/播客五板块，40 路信源+X 35 账号+小宇宙 8 播客+YouTube 12 频道）+ LLM 三层质量（diversify 喂样→精选/跨源合并/分栏 tag→两段式深读）+ 播客 STT 转写（硅基流动）+ YouTube 字幕深读（cookies 提权/429 退避/无字幕摘除门/24h 延迟窗）+ 社区治理（噪声词表+语义反选+审查集合闭合+熔断）+ 失败治理（宁缺勿发+整点安全网+看门狗账 5970s）+ 跨日已见账本/政治与未成年人防护滤/字节预算自动降密度 + BCC 密送 SMTP 直发（deadline+closeOnce 全终态）+ 网页版归档与设置页 + 出站白名单/外发账本。审计链九批全闭环（设计审→DE 四轮→r-final 三轮→转写四轮→六问三轮→三拍四轮+§17 TAKEOVER→jtw 四轮→合审两轮，范德彪真 Codex 全程）+ guardian PASS + 八封真发 degraded=false + 91 commits squash 合入 | 2026-07-12 | [F037](features/F037-daily-news-digest.md) |
