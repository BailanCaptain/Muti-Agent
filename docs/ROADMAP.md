...
| F023 | 三家 MCP 挂载统一（对齐 clowder-ai）+ 弃 CALLBACK_API_PROMPT：`.mcp.json` + `.codex/config.toml` + `.gemini/settings.json` 三份项目级配置 + 修 server.js 路径 bug（F003 遗留） | in-progress | 黄仁勋 | internal | [F023](features/F023-mcp-unified-mounting.md) |
