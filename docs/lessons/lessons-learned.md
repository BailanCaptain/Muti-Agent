...
### LL-042: 时间范围约束不得隐式删除既有栏目
- 状态：validated
- 更新时间：2026-07-27

- 坑：小孙说“周一只是整合周末，不是大改”，实现却把正文日期窗扩大解释为周一不抓 GitHub，并用测试锁死“不开源榜单”，导致五栏邮件缩成三栏。
- 根因：把时间型新闻流和无 `publishedAt` 的状态型榜单当成同一种内容；测试只验证了“什么被过滤”，没有反证长期栏目合同与源数量仍保持。
- 修复：周末日期门只约束 AI / 社区 / 热点 / 播客正文；GitHub 四榜作为周一状态快照照常运行；周一固定五栏，空栏只显示无链接空态。回归同时锁日期零越界、栏目五个、运行源 43 路与 GitHub-only fail-closed。
- 原理：新增范围约束时，必须把过滤维度与展示结构分开建模；“不大改”本身就是需要测试保护的兼容性合同。
- 关联：B044、B045、F037、D21
