...
| | **patch v3.4 (2026-05-23 小孙浏览器实测拍)**：AC-P3-1 拖宽 max 从 720 升 1200。理由: 大屏 720 不够, Inspector 注入 part 表 / 召回 query 列表需要更宽视野; 1200 留余地。影响: layout-store.ts MAX 改 + status-panel.tsx maxWidth + 4 测试期望更新。AC 字面 "360-1200px"。|
| | **patch v3.5 (2026-05-23 小孙浏览器实测拍)**：tab label 改名。lvl1 "system prompt" → "记忆系统" (1 级容器 = 整个 LLM 接到 context 总称); lvl2 "检视" → "system prompt" (prompt-inspector 显示 LLM 实际接的 system prompt 内容, 更直观)。key 不动 schema/walkthrough 不动，仅 UI label 改。|
| | **patch v3.6 (2026-05-23 小孙拍 evidence pack 双 judge 收口)**：两 AC 重新定义以反映 Phase 3 实际 scope vs Phase 4 边界：(1) **AC-P3-8 b** Inspector unresolved 入口 UI 推 Phase 4 — Phase 3 prompt-inspector tab 7 块不含 Coverage section, b frontend UI 与 Phase 4 PromoteModal 一同上线 (feature.md AC-P4-1 接审计闭环); Phase 3 AC-P3-8 收口仅含 a backend (POST/GET decisions + tombstone 语义)。(2) **AC-P3-9 重写** — 旧字面 "production wiring" 与实施 gap (codex j2 FAIL 实证 server.ts:239 用 createNoopAdaptiveRecallCoordinator)；改为 "**ready-for-Phase-4 wiring**: 代码 (AdaptiveRecallCoordinator + PromptAuditWriter + ProductionLevel5Sink class)、接口 (executeAdaptiveRecall 签名)、单测全 ready；server.ts boot 用 noop 等待 Phase 4 critique LLM + level2-4 backend + Level5Sink 生产实现接入后启用 enabled=true。Phase 3 验 fixture + 组件单测可通过"。AC-P3-9 收口接受 CONDITIONAL_PASS (评估为 'ready, 推 Phase 4 boot wire')。|
