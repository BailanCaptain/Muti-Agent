...
| 1 | 一句话结论 |
| 2 | 差距总览 |
| 3 | 差距一：任务路由模型——群聊接话 vs 结构化任务 |
| 4 | 差距二：Cross-Path 双路触发——德彪为什么每次回两条 |
| 5 | 差距三：上下文隔离——桂芬为什么回复了德彪 |
| 6 | 差距四：并发执行模型——全组串行 vs 每槽独立 |
| 7 | 差距五：调用隔离（F108 parentInvocationId） |
| 8 | 差距六：消息分类——进度播报 vs 正式答复 |
| 9 | 差距七：MCP 工具能力 |
| 10 | 差距八：记忆管理 |
| 11 | 差距九：A2A 去重机制的精确性 |
| 12 | **补充：两套系统的实现地图（按能力层对照）** |
| 13 | 整改路径（详细步骤）P1-P5 |
| 14 | **补充：整改实施分层计划（按系统层拆解）** |
| 15 | 差距根因总结 |
| 16 | 差距十：Skills 系统——3 个文本约定 vs 23 个结构化技能 |
| 17 | 差距十一：System Prompt——通用房间规则 vs 角色 + 安全铁律 + SOP |
| 18 | 差距十二：SOP 强制流程 vs 对话建议 |
| 19 | 补充：skills 和 system prompt 差距如何放大 A2A 混乱 |
| 20 | 差距十三：体验层机制与 A2A 有序性 |
| 21 | 体验层机制依赖的最小后端字段 |
| 22 | 更新后的差距总览（含新增三项） |
| 23 | **补充：A2A 如何保持有序协作（从触发到记忆的完整闭环）** |
| 24 | **补充：为什么 Multi-Agent 不能只靠 prompt / skills 文本约定补齐这些能力** |
| 25 | **补充：Skill 加载与 MCP 加载——两套系统是如何把能力接入给 agent 的** |
