...
- [一句话结论](#一句话结论)
- [差距总览](#差距总览)
- [差距一：任务路由模型——群聊接话 vs 结构化任务](#差距一任务路由模型群聊接话-vs-结构化任务)
- [差距二：Cross-Path 双路触发——德彪为什么每次回两条](#差距二cross-path-双路触发德彪为什么每次回两条)
- [差距三：上下文隔离——桂芬为什么回复了德彪](#差距三上下文隔离桂芬为什么回复了德彪)
- [差距四：并发执行模型——全组串行 vs 每槽独立](#差距四并发执行模型全组串行-vs-每槽独立)
- [差距五：调用隔离（F108 parentInvocationId）](#差距五调用隔离f108-parentinvocationid)
- [差距六：消息分类——进度播报 vs 正式答复](#差距六消息分类进度播报-vs-正式答复)
- [差距七：MCP 工具能力](#差距七mcp-工具能力)
- [差距八：记忆管理](#差距八记忆管理)
- [差距九：A2A 去重机制的精确性](#差距九a2a-去重机制的精确性)

**实现对照与整改**
- [补充：两套系统的实现地图（按能力层对照）](#补充两套系统的实现地图按能力层对照)
- [整改路径（详细步骤）P1-P5](#整改路径详细步骤)
- [补充：整改实施分层计划（按系统层拆解）](#补充整改实施分层计划按系统层拆解)
- [差距根因总结](#差距根因总结)

**工程与 SOP 差距**
- [差距十：Skills 系统——3 个文本约定 vs 23 个结构化技能](#差距十skills-系统3-个文本约定-vs-23-个结构化技能)
- [差距十一：System Prompt——通用房间规则 vs 角色 + 安全铁律 + SOP](#差距十一system-prompt通用房间规则-vs-角色--安全铁律--sop)
- [差距十二：SOP 强制流程 vs 对话建议](#差距十二sop-强制流程-vs-对话建议)
- [补充：skills 和 system prompt 差距如何放大 A2A 混乱](#补充skills-和-system-prompt-差距如何放大-a2a-混乱)

**体验层与汇总**
- [差距十三：体验层机制与 A2A 有序性](#差距十三体验层机制与-a2a-有序性)
- [体验层机制依赖的最小后端字段](#体验层机制依赖的最小后端字段)
- [更新后的差距总览（含新增三项）](#更新后的差距总览含新增三项)

**深度补充**
- [A2A 如何保持有序协作（从触发到记忆的完整闭环）](#补充a2a-如何保持有序协作从触发到记忆的完整闭环)
- [为什么 Multi-Agent 不能只靠 prompt / skills 补齐这些能力](#补充为什么-multi-agent-不能只靠-prompt--skills-文本约定补齐这些能力)
- [Skill 加载与 MCP 加载——两套系统如何把能力接入给 agent](#补充skill-加载与-mcp-加载两套系统是如何把能力接入给-agent-的)

**工程实现详解（附录）**
- [附录 A：MCP 注入机制详解——三只猫各自如何接入工具](#附录-a-mcp-注入机制详解三只猫各自如何接入工具)
- [附录 B：Skills 加载机制详解——manifest 驱动与按需加载](#附录-b-skills-加载机制详解manifest-驱动与按需加载)
- [附录 C：配置目录结构说明（.claude / .codex / .gemini）](#附录-c-配置目录结构说明claude--codex--gemini)
...
前面差距一到九逐项拆了协议层差距，这里把两套系统的实现方式集中收口一次。关键点不是"谁的提示词更强"，而是**谁把协作能力做成了系统能力**。

### 1. 路由与调度层

**Multi-Agent 的实现：**
- 路由入口主要是 mention 解析和 `dispatch.ts`
- 队列结构是 `Map<sessionGroupId, QueuedDispatch[]>`
- `QueuedDispatch` 里虽然有 `sourceMessageId`，但真正发给下一只 agent 的核心内容仍是三行字符串（`dispatch.ts:183-187`）
- 下一只 agent 启动后，需要自己再去读 `thread-context` 理解任务

**Clowder AI 的实现：**
- 路由入口是结构化 `QueueEntry` / worklist
- 条目天然携带 `caller`、`messageId`、`targetCats`、`intent`、`status`、`autoExecute`
- A2A 触发不是"把聊天转发出去"，而是"创建一条带所有权的任务"
- 调度层能判断"谁触发我""我属于哪条调用链""我现在是不是合法推进者"

**差距本质：** Multi-Agent 现在还是"文本接话"；Clowder AI 已经是"结构化任务路由"。
...
**Multi-Agent 的实现：**
- barrier 更接近 session group 级（`activeInvocations: Map<sessionGroupId, ...>`）
- 全组串行：一只 agent 在跑，其他 agent 等待
- 去重主要依赖 `rootTriggeredProviders`（root 级，按 Provider 粗去重）
- callback 路径和 stdout 路径都可能产生活动，语义并不彻底分离

**Clowder AI 的实现：**
- 执行单位更接近 `(threadId, catId)` slot
- slot 内互斥，不同 slot 可并行，互不干扰
- 调用链用 `parentInvocationId` 维持自己的 worklist
- 去重是按来源、状态、slot 精确处理（五层去重，见差距九）

**差距本质：** Multi-Agent 的执行隔离粒度太粗，顺序会放大上下文污染；Clowder 的隔离粒度更细，并发不等于混乱。
...
**Multi-Agent 的实现：**
- `GET /api/callbacks/thread-context` 返回 live 混合时间线（`callbacks.ts:137-148`）
- 默认思路是 recent N 条消息，无 `before_message_id`
- 后启动的 agent 看到的是"当前最新"，不是"触发当时的视野"

**Clowder AI 的实现：**
- 调度时冻结边界（ADR-008 cursorBoundaries）
- 执行时按边界组装上下文（只注入触发时刻之前的内容）
- 成功后才推进 cursor（失败不推进，防止状态漂移）
- 需要更多历史时，优先走 session digest 和 evidence search，不是堆聊天记录

**差距本质：** Multi-Agent 把上下文当滚动窗口；Clowder 把上下文当受协议约束的资源。
...
**Multi-Agent 的实现：**
- MCP 工具面较薄：`post_message` + `get_thread_context`
- Codex / Gemini 甚至不是完整 MCP，而是 callback API + HTTP 用法说明
- 长期记忆更多依赖 SQLite 消息表（`messages` 表）和 CLI `--resume`
- 房间一多、消息一长，容易退化成"读最近消息 + 凭印象接话"
...
| 类别 | 工具 |
|------|------|
| 协作状态 | `get_pending_mentions`、`list_tasks`、`update_task`、`list_threads`、`register_pr_tracking` |
| 记忆检索 | `search_evidence`、`read_session_digest`、`read_session_events`、`reflect`、`list_session_chain` |
| 协作编排 | `multi_mention`、`post_message`、`cross_post_message` |
| 富表达 | `create_rich_block`（card/checklist/diff/audio/gallery/interactive） |

记忆有三层：session digest（短期）→ evidence search（中期）→ reflect 合成洞察（长期）。

**差距本质：** Multi-Agent 的 agent 主要会聊天；Clowder 的 cat 能管理协作状态和项目知识。

---

### 5. Skills / Prompt / SOP 层

**Multi-Agent 的实现：**
- 主要是文本约定（`handoff.md`、`review.md`、`room-charter.md`）
- system prompt 是通用房间规则，三只 agent 几乎相同
- 工作流更多依赖模型自觉遵守，无强制阻断点

**Clowder AI 的实现：**
- 有 manifest 驱动的 skill 路由（23 个技能，统一 `manifest.yaml` 触发匹配）
- 有角色化 prompt（主架构师 / 代码审查 / 视觉设计各自独立）
- 有 quality-gate / review gate / merge gate 硬门
- 很多规范不是"建议"，而是"前一步不过，后一步不让进"

**差距本质：** Multi-Agent 的流程约束主要存在于文字里；Clowder 的流程约束存在于系统能力里。

---

### 小结：为什么 Clowder 更有序

把两套实现放在一起看，Clowder AI 的有序性来自六层连续能力：

1. 把触发转成结构化任务，而不是聊天文本
2. 用正确粒度的执行隔离保护任务所有权
3. 用冻结边界和检索式记忆控制上下文
4. 用 MCP 让 agent 感知协作状态
5. 用 skills / SOP / prompt 把规则前置成系统护栏
6. 用体验层把这些状态正确暴露出来

Multi-Agent 当前缺的不是"更强的模型"，而是这六层能力中的大部分。
...
前面的 P1-P5 给的是优先级顺序，这里再把整改拆成工程实施层。Multi-Agent 不是"改几句 prompt"就能补齐，而是要按数据库、API、runtime、MCP、前端五层逐步补能力。

### 第一层：数据库与数据模型

**Multi-Agent 当前状态：**
- `messages` 表只有 `role / content / thinking / createdAt`，无子类型字段
- 没有 `message_type`（所有输出都是同级 `assistant` 消息）
- 没有 `reply_to_message_id`
- 没有 `parent_invocation_id`
- `session_group` 侧没有 room mode、归档状态、摘要锚点
...
-- messages 表扩展
ALTER TABLE messages ADD COLUMN message_type TEXT NOT NULL DEFAULT 'final';
-- 取值: 'progress' | 'final' | 'handoff'
ALTER TABLE messages ADD COLUMN reply_to_message_id TEXT;
ALTER TABLE messages ADD COLUMN parent_invocation_id TEXT;
ALTER TABLE messages ADD COLUMN context_anchor_message_id TEXT;

-- session_groups 表扩展
ALTER TABLE session_groups ADD COLUMN status TEXT NOT NULL DEFAULT 'active';
-- 取值: 'active' | 'archived'
ALTER TABLE session_groups ADD COLUMN mode TEXT NOT NULL DEFAULT 'full-auto';
-- 取值: 'tutorial' | 'restricted' | 'full-auto'
...
CREATE TABLE session_digests (
  ...
  sealed_at TEXT NOT NULL,     -- 封存时间
  anchor_message_id TEXT       -- 摘要对应的上下文边界
);
```

**这一层补齐后，系统才第一次拥有"状态"而不只是"文本"。**

---

### 第二层：API 与协议层

**Multi-Agent 当前状态（关键代码位置）：**
- `callbacks.ts:72` — `appendMessage` 无 `messageType` 字段
- `callbacks.ts:137-148` — `thread-context` 返回 live 混合时间线，无 `before_message_id`
- `dispatch.ts:183-187` — dispatch payload 是三行字符串，无 `taskSnippet` / `replyToMessageId`

**需要补齐：**

```typescript
// 1. POST /api/callbacks/post-message — CallbackBody 扩展（callbacks.ts:8）
type CallbackBody = {
  invocationId?: string
  callbackToken?: string
  content?: string
  messageType?: 'progress' | 'final' | 'handoff'  // 新增
  replyToMessageId?: string                         // 新增
  parentInvocationId?: string                       // 新增
}

// 2. GET /api/callbacks/thread-context — 新增参数
// ?before_message_id=<id>   只返回该消息之前的内容（上下文边界）
// ?include_progress=false   默认排除 progress 消息（避免污染上下文）

// 3. QueuedDispatch 扩展（dispatch.ts:15）
export type QueuedDispatch = {
  // ...现有字段...
  taskSnippet?: string        // 新增：专门针对该 target 的任务片段
  replyToMessageId?: string   // 新增：agent 应该回复的消息 ID
  caller?: string             // 新增：调用者 provider 标识
  contextAnchor?: string      // 新增：上下文边界消息 ID
}
```

**这一层补齐后，A2A 才从"群聊转发"升级成"协议驱动"。**
...
**Multi-Agent 当前状态（关键代码位置）：**
- `message-service.ts:455` — `flushDispatchQueue` 全组串行互斥
- `message-service.ts:409-419` — stdout 路径 A2A 触发（无跨路径去重）
- `message-service.ts:148-178` — callback 路径 A2A 触发（同上）
- `dispatch.ts:140,160` — `rootTriggeredProviders` root 级粗去重

**需要补齐：**

```typescript
// 1. 双路触发修复（message-service.ts）
private readonly callbackPostedInvocations = new Set<string>()

// handleAgentPublicMessage 里，入队后标记
this.callbackPostedInvocations.add(options.invocationId)

// runThreadTurn 结束时，stdout 路径检查
const hasCallbackMessage = this.callbackPostedInvocations.has(identity.invocationId)
if (!hasCallbackMessage) {
  // 正常走 stdout 路径，触发 A2A
} else {
  // callback 已发：stdout 只保存 thinking，不触发 A2A
}

// 2. Slot 级互斥（dispatch.ts）
// 把 activeInvocations: Map<sessionGroupId, Set> 改成
// Map<`${sessionGroupId}:${provider}`, Set>
// takeNextQueuedDispatch 只检查目标 provider 的 slot 是否空闲

// 3. Caller-position guard（invocation-registry.ts 附近）
// 引入 parentInvocationId 作为 worklist 隔离键
// 旧 invocation 的 callback 在 worklist 已移动后被拒绝推进
```

**这一层补齐后，Multi-Agent 才第一次具备真正的有序并发，德彪双发和桂芬漂移都能根治。**
...
**Multi-Agent 当前状态：**
- `mcp/server.ts` 只暴露 `post_message` 和 `get_thread_context`
- session 结束后没有自动 seal / digest 流程
- 没有"待处理 mention""任务状态""历史摘要"的一等公民接口
...
// mcp/server.ts — 新增工具
{
  ...
  description: "获取当前 session group 中待处理的 @提及",
  inputSchema: { properties: { sessionGroupId: { type: "string" } } }
}
{
  name: "list_tasks",
  description: "列出当前 thread 的任务列表及状态",
  inputSchema: { properties: { threadId: { type: "string" }, status: { type: "string" } } }
}
{
  name: "update_task",
  description: "更新任务状态",
  inputSchema: { properties: { taskId: { type: "string" }, status: { type: "string" } } }
}
{
  name: "read_session_digest",
  description: "读取封存后的 session 摘要",
  inputSchema: { properties: { sessionGroupId: { type: "string" } } }
}
{
  name: "search_evidence",
  description: "在项目知识库中搜索历史决策和教训",
  inputSchema: { properties: { query: { type: "string" } } }
}
```

**配套后端改动：**
- session 结束时自动 seal + 生成 digest，写入 `session_digests` 表
- 为 `messages` 表建立全文索引，支持 evidence 检索
- `room mode` 影响 agent 的 `a2aDepthLimit`（初始 2，用户解锁后逐步放开）

**这一层补齐后，agent 才能从"只能聊天"升级成"能管理协作状态和项目记忆"。**
...
**需要补齐：**
- `progress` 渲染为时间线事件流（灰色小字或 spinner），而不是聊天气泡
- `final` 才进入主气泡流
- `handoff` 渲染为任务卡片（显示 from / to / taskSnippet / 状态灯）
- 连续多轮（建议 3 轮）无人类干预的 A2A 后，插入交互式 check-in prompt
- room mode（`tutorial` / `restricted` / `full-auto`）驱动 UI 能力渐进解锁

**这一层补齐后，用户才会"感觉系统有序"；但它依赖前四层，不是第一步。**
...
```
第一层：数据模型（messageType / replyTo / parentInvocationId / roomMode）
    ↓
第二层：API 协议层（before_message_id / taskSnippet / progress|final）
    ↓
第三层：调度器与 runtime（双路触发修复 / slot 互斥 / caller guard）
    ↓
第四层：MCP 与记忆层（pending mentions / task state / session digest / evidence）
    ↓
第五层：前端体验层（任务卡片 / 进度流 / check-in / room mode UI）
```

这个顺序不能反。A2A 混乱首先是协议问题，其次才是交互问题。在协议层没有正确建模之前，任何体验层改动都只是把混乱渲染得更漂亮。
...
烁烁的三个 UX 建议不只是前端样式改动——如果没有协议层的字段支撑，它们都会退化成"把原始文本重新画漂亮一点"的表面工程。以下是每个体验机制需要的最小后端前置条件，以及它们对应的整改路径节点：

| 体验机制 | 依赖的后端字段 / 状态 | 当前缺失位置 | 对应整改路径 |
|---------|---------------------|------------|------------|
| **进度流 vs 正式答复** | `messageType: 'progress' \| 'final'` | `callbacks.ts:72` — appendMessage 无类型字段；`callbacks.ts:137` — thread-context 不返回类型 | P1（callbackMessagePosted 互斥） + P4（progress/final 分离） |
| **任务交接卡片** | dispatch envelope 中的 `from / to / replyToMessageId / taskSnippet` | `dispatch.ts:183` — 三行纯字符串，无结构字段 | P3（taskSnippet + replyToMessageId） |
| **check-in 暂停块** | `session_group` 级的 `roomMode` 状态位（`running \| check-in-pending \| paused`） | `flushDispatchQueue`（`message-service.ts:455`）无"暂停等确认"状态，会自动跑到队列空 | P5（per-cat slot 并发，同步引入 room mode） |
| **能力边界渐进解锁** | Thread 级的 `a2aDepthLimit`（初始 2，用户确认后逐步放开至 15） | `dispatch.ts:44` MAX_HOPS 固定 15，无动态覆盖机制 | P5 附带改动 |

**核心结论**：体验层改动不是独立工程，它是协议层整改的自然延伸。先做 P1-P3（`messageType` + `taskSnippet` + `before_message_id`），进度流/任务卡片/上下文冻结就有了后端支撑；P4-P5（消息分类 + room mode）上线后，check-in 暂停和能力边界管控才能真正挂起链路而不只是弱提示。
...
## 补充：A2A 如何保持有序协作（从触发到记忆的完整闭环）

一次 A2A 触发从发生，到被正确分发，到执行，到回流，到去重，到沉淀为记忆，中间每一步都必须有明确的所有权、状态和边界。Clowder AI 的有序来自这六步闭环；Multi-Agent 当前的混乱也分别对应这六步中的缺口。

### 1. 触发（Trigger）

**Multi-Agent：**
- 触发主要来自文本里的 `@mention` 字符串匹配
- 系统知道"你提到了谁"，但不知道"你把什么任务交给了谁"
- 一条消息里同时提到多只 agent 时，没有天然的任务拆分语义

**Clowder AI：**
- 触发不是纯文本事件，而是一个可路由的任务信号
- 系统同时知道：触发者 / 被触发者 / 触发消息 ID / 所属调用链 / 任务意图
- `autoExecute: true` 标记区分 A2A 条目和用户请求条目

**有序协作的关键：** A2A 的起点不能只是"看到了一个 @"，而必须是"创建了一条带所有权的任务"。
...
**Multi-Agent：**
- 分发载荷是三行模糊字符串（`dispatch.ts:183-187`）
- 下一只 agent 启动后需要自己去聊天记录里理解任务
- 分发阶段没有真正完成"任务绑定"

**Clowder AI：**
- 分发条目本身带有任务语义（target / caller / taskSnippet / contextAnchor）
- 被分发者启动时，不需要先猜"我该处理哪一段"
- 分发阶段已经完成：目标绑定 + 来源绑定 + 调用链绑定 + 任务片段绑定 + 上下文边界绑定

**有序协作的关键：** 分发如果只是"把上一条话转述一下"，那就还是群聊；分发必须把任务结构化，才算 A2A。
...
**Multi-Agent：**
- 执行 barrier 偏 group 级（`message-service.ts:455`，全组串行）
- 后启动的 agent 往往会看到前一只 agent 已经写入的最新消息
- 上下文是"执行当时的房间现状"，不是"触发当时的视野"

**Clowder AI：**
- 执行粒度偏 slot 级，不同 cat 在正确的边界内独立运行
- 每只 cat 启动时看到的是被系统控制过的上下文，不是 live 群聊流
- 系统能判断当前 invocation 是否仍拥有推进下一跳的资格（caller-position guard）

**有序协作的关键：** 执行阶段必须保护"任务所有权"和"视野边界"，否则后执行者天然会被前执行者带偏。

---

### 4. 回流（Return / Callback）

**Multi-Agent：**
- callback 路径和 stdout 路径都可能回流信息（部分场景双路）
- 没有消息类型区分，系统和用户都不知道哪些是进度、哪些是结果
- `codex-runtime.ts:95-101` 中 agent_message delta 和完成态都并入 assistant delta，放大"回两次"的感知

**Clowder AI：**
- 回流内容是有类型的：`progress` / `final` / `a2a_handoff` 走不同通道
- 系统可以决定：哪些只显示为过程态，哪些进入正式时间线，哪些应继续触发下一跳
- cross-path 去重确保 callback 路径和文本扫描路径不会双触

**有序协作的关键：** 回流不是"只要有文本就往房间里丢"，而是"不同类型的输出走不同的通道"。

---

### 5. 去重与流控（Dedup / Flow Control）

**Multi-Agent：**
- 去重偏 root 级，粒度较粗（`rootTriggeredProviders`）
- 副作用：会误杀同一 root 下的合法二次回合
- 流控更多是被动限流（MAX_HOPS=15 硬熔断），无主动流程检查点

**Clowder AI：**
- 去重结合来源、状态、slot、调用链精确判断（五层去重）
- 旧 invocation、重复 handoff、处理中目标，都能被精确识别
- `hyperfocus-brake` 提供主动节奏断路器（L1/L2/L3 升级 + 交互式确认）

**有序协作的关键：** 真正的有序不是"尽量少重复"，而是"系统知道什么叫重复，什么叫合法的下一轮"。

---

### 6. 记忆与续接（Memory / Continuity）

**Multi-Agent：**
- 续接更多依赖 recent messages 和 CLI `--resume`
- 房间一多、对话一长，容易丢历史决策
- agent 无法稳定回答"我们之前在这个问题上已经做到哪了"

**Clowder AI：**
- 短期续接靠当前上下文边界（cursorBoundaries）
- 中期续接靠 session digest（`read_session_digest`）
- 长期续接靠 evidence search / reflect / session chain
- agent 不必重读所有聊天，也能把历史决定带回当前回合

**有序协作的关键：** A2A 不只是"这一跳别乱"，还包括"下一轮别忘"。没有记忆层，协作就只能停留在短会话尺度。

---

### 闭环总结

把这六步连起来，Clowder AI 的 A2A 实际上是一条受控流水线：

```
触发 → 创建结构化任务（带所有权 + 意图）
分发 → 绑定 taskSnippet + contextAnchor + caller
执行 → 保护 slot 合法性 + 视野边界
回流 → 按消息类型分通道处理（progress/final/handoff）
去重 → 按来源、状态、调用链精确判定
记忆 → 沉淀为 digest + evidence，供下一轮检索
```

Multi-Agent 当前的问题，并不是某一步"偶尔做错"，而是这六步里有多步都还停留在"文本协作"层，没有上升到"协议协作"层。所谓"让 A2A 保持有序"，本质上不是优化对话风格，而是把这六步全部做成系统能力。

---

## 补充：为什么 Multi-Agent 不能只靠 prompt / skills 文本约定补齐这些能力

看到这里，一个很自然的问题是：是不是只要把 prompt 改强一点、把 skills 文档写细一点，就能让 Multi-Agent 接近 Clowder AI？

答案是否定的。**prompt 和 skills 文本只能影响"模型如何表达"，不能凭空创造"系统如何约束执行"的能力。** Prompt 可以改变模型的倾向，但不能替代协议、状态机、数据库字段、调度器、工具接口和记忆系统。

### 1. Prompt 不能创造任务所有权

Multi-Agent 里，"下一只 agent 不知道自己到底该回谁"的根因不是模型不够听话，而是系统没有把以下信息结构化交给它：触发者是谁、我正在回复哪条消息、这一轮专门分给我的任务片段是什么、我是不是当前仍合法的推进者。

如果这些字段不存在，那么 prompt 再怎么强调"请只回答分配给你的问题"，模型也只能从聊天记录里猜。

**结论：** 没有 `replyToMessageId`、`taskSnippet`、`caller` 这些字段，prompt 只能让 agent 猜得更努力，不能让它真正知道答案。

### 2. Prompt 不能冻结上下文边界

只要系统把"当前最新消息"返回给 agent，后执行者就一定会受到前执行者影响。你可以在提示词里写"不要被其他 agent 的回复干扰"，但只要系统没有 `before_message_id`、没有按触发时刻冻结视野，模型面对的输入仍然是混合过的。

**结论：** 上下文边界是 API 和调度器能力，不是语言能力。

### 3. Prompt 不能解决双路径语义冲突

如果系统不区分 `progress` 和 `final`，那么 prompt 再怎么写"中间过程不要当最终答复"，也无法阻止运行时把不同语义的内容都落成普通 assistant 消息。什么时候该显示为进度、什么时候该算正式答复，必须由协议层定义，而不是交给模型自觉。

### 4. Skills 文本不能替代 MCP 工具能力

如果 skill 里写"先搜后问"，那系统必须真的有 `search_evidence`。如果 skill 里写"完成后更新状态"，那系统必须真的有 `update_task`。如果 skill 里写"查清楚 pending 再回复"，系统必须真的有 `get_pending_mentions`。

没有工具，skill 只是更长的说明书。

### 5. Skills 文本不能替代记忆系统

如果系统没有 digest、没有 evidence search、没有 session chain，那么"先回顾历史"最终还是退化成"读最近几十条消息"。历史太长时模型读不全，历史太多时模型自行压缩质量不稳定。

**结论：** "请记住历史"不是记忆系统；真正的记忆系统必须提供摘要、索引和检索。

### 6. Prompt 不能替代硬门和流程阻断

Clowder AI 的秩序感，其实不是来自 cat 更守规矩，而是来自系统有硬门：quality-gate 不过不能进 review，review 没过不能 merge，handoff 缺字段不能视为完整交接，room mode 不允许就不能无限制 A2A。这些约束如果只写在 prompt 里，就会退化成"建议你这样做"。

### 最终判断：Prompt 负责表达，系统能力负责秩序

- **Prompt** 决定模型怎么说
- **Skill** 决定模型被建议怎么做
- **协议、状态机、工具、记忆、gate** 决定系统实际允许什么发生

Clowder AI 的有序，是因为后三者齐了。Multi-Agent 当前的瓶颈，是后三者缺得太多。把整改方向理解成"继续优化 prompt"，会得到一个**更会描述秩序的系统**，而不是**真正更有秩序的系统**。

这也是为什么整改建议始终强调：先补协议字段 → 再补调度与隔离 → 再补 MCP 和记忆 → 最后才谈 prompt、skills 和 UI。在多智能体系统里，**秩序首先是工程能力，其次才是语言能力。**

---

## 补充：Skill 加载与 MCP 加载——两套系统是如何把能力接入给 agent 的

Skill 和 MCP 如果分开看，读者容易漏掉一个关键问题：**系统到底是怎么把这些能力"装载"给 agent 的？**

Clowder AI 的 skill 和 MCP 都是运行时能力面的一部分；Multi-Agent 的很多 skill 更像文档，很多工具更像补充接口。换句话说，Clowder AI 是"先把能力接入，再让 agent 使用"；Multi-Agent 更像"先写说明文档，再期待 agent 主动遵守"。

### 一、Skill 的加载方式

**Multi-Agent：以文本约定为主，缺少统一加载器**

Multi-Agent 的 skills 主要表现为几份 markdown 文档（`handoff.md`、`review.md`、`system/room-charter.md`）。它们的问题不是内容写得差，而是加载方式不够系统化：

- 没有统一 manifest（无路由配置，无触发短语匹配，无技能注册表）
- 没有明确的 trigger matcher
- 没有"这个场景必须加载哪个 skill"的强约束
- 没有加载后状态跟踪
- 没有 step-by-step 的阻断点

结果是：skill 更像"供 agent 阅读的建议"，是否加载、何时加载、加载到什么深度，很大程度靠模型自己判断。即使文档里写了 review/handoff 规范，系统也不会因为 agent 跳过而阻止后续流程。

**Clowder AI：skill 是受路由规则驱动的运行时能力**

Clowder AI 的 skill 加载链路：

1. **运行时先暴露可用 skill 列表**：agent 在会话一开始就能看到有哪些 skill、描述、用途和文件路径
2. **有触发规则**：manifest.yaml 里每个 skill 有 `triggers` 触发短语列表，匹配到就必须加载，而不是"看心情"
3. **按需读取 SKILL.md**：不是一次性把所有 skill 全读进 prompt，而是按当前任务读取对应 skill
4. **渐进式加载引用材料**：skill 若引用 `refs/`、脚本、模板，只加载完成当前任务所需的那部分
5. **skill 之间能形成链路**：`feat-lifecycle → writing-plans → worktree → tdd → quality-gate → request-review → receive-review → merge-gate`，每个节点的产物是下一个节点的输入验证

因此在 Clowder AI 里，skill 的价值不只是"告诉 agent 应该怎么做"，而是：告诉 agent **现在必须用什么能力**、**下一步该切到哪个能力**、**哪些情况下不该用这个能力**。

### 二、MCP 的加载方式

**Multi-Agent：MCP 更像房间回调通道**

Multi-Agent 的 MCP 加载比较轻量：
- Claude runtime 用 `--mcp-config` 内联一个 `multi_agent_room` server
- 通过环境变量注入 `API_URL / invocationId / callbackToken`（`claude-runtime.ts:75`）
- 工具面只有 `post_message` + `get_thread_context`（`mcp/server.ts`）
- Codex / Gemini 甚至不走完整 MCP，而是 `CALLBACK_API_PROMPT` + HTTP 用法说明（`agent-prompts.ts:30`）

这种设计能解决"agent 如何发消息、如何读最近上下文"，但解决不了：我还有没有 pending mention、当前任务状态是什么、这个房间上次的总结是什么、当前链路是不是已被别人接管。

**Clowder AI：MCP 是完整的能力平面**

Clowder AI 的 MCP 是多命名空间、多类别的工具平面，按能力分五大类：

| 类别 | 工具 |
|------|------|
| 协作类 | `post_message` / `get_thread_context` / `get_pending_mentions` / `multi_mention` / `update_task` / `list_tasks` / `register_pr_tracking` |
| 记忆类 | `search_evidence` / `read_session_digest` / `read_session_events` / `reflect` / `list_session_chain` / `read_invocation_detail` |
| 富表达类 | `create_rich_block`（card/checklist/diff/audio/gallery/interactive） |
| 信号类 | article / study / podcast 系列（来自 cat-cafe-signals） |
| 权限/治理类 | `request_permission` / `update_workflow` / `update_bootcamp_state` |

因此在 Clowder AI 里，MCP 不只是"让 agent 调接口"，而是把整个协作系统的关键状态都通过工具暴露出来。cat 可以在执行前先做：先搜 evidence 再决定方向、先查 pending mentions 再决定优先级、先读 session digest 再决定是否需要重读原始线程。

### 三、为什么 Skill 加载和 MCP 加载必须一起看

只看 skill，不看 MCP：会误以为"把规则写细一点就行"。
只看 MCP，不看 skill：又会误以为"工具多一点就行"。

实际上两者要配套：

- **Skill 负责告诉 agent 什么时候该做什么**
- **MCP 负责给 agent 真正可执行的能力**
- **System Prompt / SOP 负责规定顺序和边界**
- **调度器和数据模型负责保证这些动作真的成立**

如果 skill 里写"先搜后问"，那系统必须真的有 `search_evidence`。如果 skill 里写"等待下一棒确认"，那系统必须真的有任务、交接、消息分类这些状态语义。否则 skill 再完整，也只是更长的说明书。

### 四、结论

从加载机制看，两套系统的差距可以概括成一句话：

- **Multi-Agent**：把能力更多写在文档和 prompt 里，再让 agent 自己理解和调用
- **Clowder AI**：把能力先做成运行时可见、可路由、可调用、可阻断的模块，再让 agent 在其上协作

这就是为什么 Clowder AI 的 skill 和 MCP 会直接提高 A2A 的有序性，而 Multi-Agent 的很多规范目前还停留在"知道应该这么做"，却没有变成"系统保证能这么做"。

---

## 附录 A：MCP 注入机制详解——三只猫各自如何接入工具

Clowder AI 的三只猫在接入 MCP 工具时方式各不相同，由每只猫的 CLI 支持程度决定。

### Claude（原生 MCP 支持）

**代码位置**：`ClaudeAgentService.ts:193`

Claude CLI 原生支持 `--mcp-config` 参数，直接传 JSON 格式的 MCP server 配置：
...
MCP server 路径由 `resolveDefaultClaudeMcpServerPath()`（`ClaudeAgentService.ts:115`）解析，定位 `packages/mcp-server/dist/index.js`。环境变量（`CAT_CAFE_API_URL`、`CAT_CAFE_INVOCATION_ID`、`CAT_CAFE_CALLBACK_TOKEN` 等）通过继承进程环境注入。

---

### Codex（TOML 逐行注入）

**代码位置**：`CodexAgentService.ts:143` 的 `buildCatCafeMcpConfigArgs()`

Codex CLI 不支持单次 `--mcp-config` JSON，但支持 `--config key=value` 的 TOML 路径写法，因此改成逐行注入：

```typescript
// MCP server 基础配置
args.push('--config', 'mcp_servers.cat-cafe.command="node"')
args.push('--config', `mcp_servers.cat-cafe.args=[${toTomlString(serverPath)}]`)
args.push('--config', 'mcp_servers.cat-cafe.enabled=true')

// 运行时环境变量（每次 invocation 都不同，不能写死在静态配置里）
const callbackKeys = [
  'CAT_CAFE_API_URL', 'CAT_CAFE_INVOCATION_ID', 'CAT_CAFE_CALLBACK_TOKEN',
  'CAT_CAFE_USER_ID', 'CAT_CAFE_SIGNAL_USER'
]
...
`toTomlString()` 函数（`CodexAgentService.ts:114`）处理 TOML 字符串转义（反斜杠、引号等特殊字符），确保注入的值不破坏 TOML 语法。

---

### Gemini（无 MCP 注入 → HTTP callback fallback）

**代码位置**：`GeminiAgentService.ts:79`、`McpPromptInjector.ts`

Gemini CLI 对 MCP 支持有限，API 层**不注入 MCP 配置**，改用 `McpPromptInjector.ts` 的 HTTP callback fallback 方案：

```typescript
// McpPromptInjector.ts
export function needsMcpInjection(mcpAvailable: boolean): boolean {
  return !mcpAvailable  // Gemini 运行时 mcpAvailable=false，触发 HTTP 指令注入
}
...
可用工具: post-message / register-pr-tracking / search-evidence / reflect / ...
完整用法: GET $CAT_CAFE_API_URL/api/callbacks/instructions`
}
```

注入后，Gemini 通过 HTTP 请求 `$CAT_CAFE_API_URL/api/callbacks/...` 端点实现协作，而不是调用 MCP 工具。能力面不如 Claude/Codex（无法调用 `create_rich_block`、`multi_mention` 等 MCP 专属工具），但核心功能（发消息、读上下文、搜索记忆）都能走 HTTP 实现。
...
| 猫 | 注入方式 | 工具调用协议 | 能力完整性 |
|----|---------|------------|----------|
| Claude | `--mcp-config` JSON（原生） | MCP JSON-RPC | 完整（10+ 工具） |
| Codex | `--config` 逐行 TOML | MCP JSON-RPC | 完整（同上） |
| Gemini | prompt 内 HTTP 说明（fallback） | HTTP REST | 基础（发消息/读/搜索） |

---

## 附录 B：Skills 加载机制详解——manifest 驱动与按需加载

### manifest.yaml 是唯一真相源

`cat-cafe-skills/manifest.yaml` 定义了所有技能的元数据：
- `skills.<name>.description`：含 "Use when / Not for / Output" 三段的触发条件（CSO 格式）
- `skills.<name>.triggers`：触发短语列表
- `iron_laws`：三条铁律（数据保护 / 进程自保 / 配置不可变）
- `sop_navigation`：SOP 阶段跳转表
- `lint`：代码规范约束

### skills 目录如何被三只猫找到

**代码位置**：`capabilities.ts:186` 的 `resolveCatCafeSkillsSourceDir()`
...
从项目根目录向上最多找 5 层，找到包含 `cat-cafe-skills/manifest.yaml` 的目录即为 skills 根目录。不管三只猫的工作目录在哪个层级，都能定位到同一份 skills，不需要各自维护一份副本。

### manifest 的解析

**代码位置**：`capabilities.ts:323` 的 `parseManifestSkillMeta()`

解析 manifest.yaml，提取每个 skill 的 description（CSO 格式）和 triggers（触发短语列表），通过 CapabilityBoard API 返回前端展示可用 skill 列表。

### 三个 CLI 的 skill 注入方式

Skills 不是一次性全部塞进 prompt，而是**按需加载**：

| CLI | skill 注入方式 |
|----|-------------|
| Claude | `--append-system-prompt`，SystemPromptBuilder 构建身份串，含 SOP 阶段提示（`SOP: F0xx stage=dev → load skill: tdd`） |
| Codex | `buildStaticIdentity()` 返回值前置到 prompt |
| Gemini | 同 Codex，前置到 prompt |

**SOP 阶段提示注入**（`SystemPromptBuilder.ts:523`）：
...
系统在 prompt 里插入 `SOP: F042 stage=review → load skill: receive-review`，告诉 cat 当前应该加载哪个 skill，而不是让 cat 自己从所有 skills 里猜。
...
1. 触发词 / 显式调用 / SOP 阶段匹配
   ↓
2. manifest.yaml triggers 匹配 → 确定 skill 名称
   ↓
3. 读取 cat-cafe-skills/<skill-name>/SKILL.md
   ↓
4. SKILL.md 内容注入到当前 invocation 的 prompt
   ↓
5. refs/ 引用（模板/规范等）按需追加读取
```

这和 Multi-Agent "把说明文档一次性放进 system prompt" 的做法本质不同：Multi-Agent 的 skill 是静态注入，Clowder AI 的 skill 是动态按需加载，不会无谓地占用上下文窗口。

---

## 附录 C：配置目录结构说明（.claude / .codex / .gemini）

三只猫的运行时配置分别存放在项目根目录下的三个隐藏目录：

```
clowder-ai/
├── .claude/
│   └── settings.local.json     # Claude CLI 本地配置（权限白名单 + MCP 启用列表）
├── .codex/
│   └── config.toml             # Codex CLI MCP 服务器静态配置
└── .gemini/
    └── settings.json           # Gemini CLI MCP 服务器配置（带环境变量占位符）
```
...
- `permissions.allow`：Claude 在交互/自动模式下无需每次弹出确认的命令白名单
- `enabledMcpjsonServers`：启用的 MCP server 名称（对应项目根 `.mcp.json` 里的 server 定义）

MCP server 实际定义在 `.mcp.json`，`settings.local.json` 只负责启用/禁用的开关。
...
command = "node"
args = ["C:\\Users\\...\\packages\\mcp-server\\dist\\collab.js"]
enabled = true

[mcp_servers.cat-cafe-memory]
command = "node"
args = ["C:\\Users\\...\\packages\\mcp-server\\dist\\memory.js"]
enabled = true

[mcp_servers.cat-cafe-signals]
command = "node"
args = ["C:\\Users\\...\\packages\\mcp-server\\dist\\signals.js"]
enabled = true
```

Codex 在**交互模式**下加载的静态配置。API 层的动态注入（`buildCatCafeMcpConfigArgs()`）在程序化调用时通过 `--config` 参数**覆盖/补充**这份配置中的环境变量部分——`invocationId`、`callbackToken` 等每次都不同，不适合写死在 TOML 文件里。
...
`${VAR}` 占位符在 Gemini CLI 启动时由进程环境变量填充。`writeGeminiMcpConfig()`（`mcp-config-adapters.ts`）写入时：
- 跳过 pencil server（避免与其他 MCP server 冲突）
- 确保 env 字段保持占位符格式，而不是写死运行时值

### 配置写入时机

三个目录的配置文件由 `governance-bootstrap.ts` 在项目初始化时生成/更新：
- `writeClaudeMcpConfig()`：保留用户自定义的非 cat-cafe server
- `writeCodexMcpConfig()`：合并模式，保留非管理 server
- `writeGeminiMcpConfig()`：跳过冲突项，确保 env 占位符格式

更新逻辑是**合并而非覆盖**：保持 Clowder AI 自己的 MCP 配置最新，同时不破坏用户在同一 CLI 下配置的其他 MCP server。
