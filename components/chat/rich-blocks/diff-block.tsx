...
export function DiffBlockComponent({ block }: { block: DiffBlock }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200">
      <div className="border-b border-slate-200 bg-slate-100 px-4 py-1.5 font-mono text-[11px] text-slate-500 truncate">
        {block.filePath}
      </div>
      <pre className="overflow-x-auto bg-surface-canvas px-4 py-2 font-mono text-[12px] leading-5">
        {lines.map((line, i) => (
