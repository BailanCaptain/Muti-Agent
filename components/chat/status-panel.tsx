...
export function StatusPanel() {
  ...
  const providerEntries = Object.entries(providers) as Array<[Provider, (typeof providers)[Provider]]>;
  const isAnyRunning = providerEntries.some(([, provider]) => provider.running);

  const modeLabel =
    socketState === "connected" ? (isAnyRunning ? "Agents running" : "Ready") : socketState === "error" ? "Connection error" : "Offline";

  // Heuristic counters keep the stats card informative until richer backend analytics land.
  const stats = {
    total: timeline.length,
    agents: timeline.filter((message) => message.role === "assistant" && message.alias !== "System").length,
    system: timeline.filter((message) => message.alias === "System").length,
    evidence: timeline.filter((message) => /(https?:\/\/|```|^\s*>|\|.+\|)/m.test(`${message.content}\n${message.thinking ?? ""}`)).length,
  ...
  return (
    <aside className="flex h-screen w-[340px] flex-col border-l border-slate-200/80 bg-[linear-gradient(180deg,rgba(248,250,252,0.96),rgba(255,255,255,0.86))] px-5 py-6 shadow-[-18px_0_48px_rgba(15,23,42,0.04)] backdrop-blur-xl">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-slate-400">Control Panel</p>
          <h2 className="mt-2 text-xl font-semibold tracking-[0.01em] text-slate-900">Status Board</h2>
          <p className="mt-1 text-xs text-slate-400">Current mode: {modeLabel}</p>
        </div>
        <button className="rounded-full border border-slate-200/70 bg-white/90 p-2.5 shadow-sm transition hover:bg-slate-50" type="button">
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-400">
              {isAnyRunning ? "At least one agent is streaming or processing." : "No active runs. The room is waiting for the next prompt."}
            </p>
          </div>
          <div
            className={`mt-1 h-2.5 w-2.5 rounded-full ${
              socketState === "connected" ? (isAnyRunning ? "animate-pulse bg-orange-500" : "bg-emerald-500") : "bg-slate-300"
            }`}
          />
        </div>
      ...
      <div className={`${panelClassName} mb-5`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          {metricCards.map((item) => (
            <div className="rounded-2xl border border-slate-200/70 bg-slate-50/80 px-3 py-3" key={item.label}>
              <div className="text-[11px] uppercase tracking-[0.18em] text-slate-400">{item.label}</div>
              <div className="mt-1 font-mono text-lg font-semibold text-slate-900">{item.value}</div>
            </div>
          ))}
        </div>
      </div>
                ...
                <div className="mb-3 flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <div className="truncate text-sm font-semibold text-slate-800">{card.alias}</div>
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${theme.badge}`}>{provider}</span>
                    </div>
                    <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                      <span className={`h-2 w-2 rounded-full ${card.running ? theme.dot : "bg-slate-300"}`} />
      ...
      <div className={`${panelClassName} min-h-0 flex-1`}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="font-mono text-[11px] text-slate-400">{sortedInvocationStats.length} sessions</span>
        </div>
        ...
        <div className="space-y-3 overflow-y-auto pr-1">
          {sortedInvocationStats.length === 0 ? (
            ...
            sortedInvocationStats.map((stat) => {
              ...
              return (
                <div className="rounded-[24px] border border-slate-200/70 bg-slate-50/80 p-3.5" key={stat.sessionId}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${theme.badge}`}>{stat.provider}</span>
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${statusTheme.badge}`}>
                  ...
                  <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
                    ...
                    <span className="font-mono text-emerald-600">cache {formatCachePercent(stat.cachedTokens, stat.inputTokens)}%</span>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    <div className="rounded-2xl bg-white px-2.5 py-2">
                      ...
                      <div className="mt-1 font-mono text-[11px] font-semibold text-slate-800">{formatTokenCount(stat.inputTokens)}</div>
                    </div>
                    <div className="rounded-2xl bg-white px-2.5 py-2">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-slate-400">Out</div>
                      <div className="mt-1 font-mono text-[11px] font-semibold text-slate-800">{formatTokenCount(stat.outputTokens)}</div>
                    </div>
                    <div className="rounded-2xl bg-white px-2.5 py-2">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-slate-400">Total</div>
                      <div className="mt-1 font-mono text-[11px] font-semibold text-slate-800">{formatTokenCount(totalTokens)}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className="font-mono text-[10px] text-slate-400">{Math.round(outputRatio)}%</span>
                  </div>
                </div>
              );
            })
